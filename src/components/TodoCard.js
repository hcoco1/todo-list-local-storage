...
function TodoCard({ todo, onDelete }) {
  ...
  return (
    <MDBCard border='primary' background='light' className='text-black mb-3'>
      <MDBCardHeader className="d-flex justify-content-between" tag="h6">
        <span
        ...
        <span
          style={{ cursor: 'pointer' }}          > 
          <strong>Created at: </strong>{todo.createdAt} {/* Change 'otherField' to the actual field you want to display */}
        </span>
      </MDBCardHeader>
      <MDBCardBody>
        <MDBCardSubTitle className="mb-1 text-start">
          {/* Here, showing the period, AFE, process, and error without copy functionality */}
          <strong>- Period:</strong> {todo.period} <strong>- AFE:</strong> {todo.afe} <strong>- Process:</strong> {todo.processPath} <strong>- Error:</strong> {todo.error}
        </MDBCardSubTitle>
        <MDBCardText>
          <span
            style={{ cursor: 'pointer' }}
